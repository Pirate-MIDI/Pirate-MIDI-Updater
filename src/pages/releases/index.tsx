...
function Releases({ devices }: { devices: ConnectedDevice[] }) {
  ...
  const device: ConnectedDevice = devices.find(
    (d) => d.serial_number === router.query.serial_number,
  );

  const onRemoteInstall = async (
  ...
  const stylePrerelease = (release) => {
    return release.prerelease
      ...
      : "bg-emerald-300 border-emerald-400 hover:bg-emerald-400";
  };
  ...
  useEffect(() => {
    ...
    if (device && device.releases) {
      ...
      let sorted_releases = device.releases.sort(
        (a, b) =>
          new Date(b.published_at).getTime() -
          new Date(a.published_at).getTime(),
      );
      setReleases(sorted_releases);
      // get the latest stable version - not beta
      setSelected(sorted_releases.find((rel) => !rel.prerelease));
    }
  }, [device]);
      ...
      <div className="flex flex-col h-screen overflow-hidden">
        <div className="flex h-5/6">
          <div
            className={
              showAllReleases
                ? "flex flex-col w-1/4 max-w-xs border-r"
                : "hidden"
            }
          >
            <ReleaseList
              releases={releases}
              selected={selected}
              onSelect={(release) => setSelected(release)}
            />
          </div>
        ...
        <div className="flex items-center justify-between p-4 border-t h-1/6">
          <button
            ...
            className="flex items-center px-4 py-2 border rounded border-slate-300"
          >
          ...
          <div className="flex">
            <button
              onClick={() => router.back()}
              className="flex items-center px-4 py-2 m-2 border rounded border-slate-300"
            >
              <ArrowLeftIcon className="icon-left" />
              Back to Device List
            </button>
            <button
              onClick={() => onRemoteInstall(device, selected)}
              className={`flex items-center px-4 py-2 m-2 border rounded text-slate-800 ${stylePrerelease(selected)}`}
            >
